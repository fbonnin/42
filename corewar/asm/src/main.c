...
/*   Updated: 2017/08/10 20:21:10 by fbonnin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			main(int argc, char **argv)
{
	...
	if (r == -1)
		return (end(&a, "", 2));
	add_value_to_bytecode(a.bytecode, COREWAR_EXEC_MAGIC, 4);
	add_value_to_bytecode(a.bytecode + 4 + PROG_NAME_LENGTH + 4,
	a.pc - 4 - PROG_NAME_LENGTH - 4 - 4 - COMMENT_LENGTH - 4, 4);
	add_strings_to_bytecode(a.bytecode, a.prog_name, a.description);
	replace_calls(a.bytecode, a.declarations, a.calls);
	write_bytecode(a.bytecode, a.pc, argv[1], len);
	return (end(&a, "Done.", 1));
}
